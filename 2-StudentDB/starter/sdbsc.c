...
int print_db(int fd) 
{
    ...
    while ((readReturn = read(fd, &student, STUDENT_RECORD_SIZE)) != 0) {
        ...
        if (memcmp(&student, &studentCmp, STUDENT_RECORD_SIZE) != 0) {
            ...
            printf(STUDENT_PRINT_FMT_STRING, student.id, student.fname, student.lname, calculatedGPA);
        }
    }

    if (!firstRowEncounter) { printf(M_DB_EMPTY); }
    return NO_ERROR;
}
...
int compress_db(int fd)
{
    ...
    while ((readReturn = read(fd, &student, STUDENT_RECORD_SIZE)) != 0) {
        ...
        if (memcmp(&student, &studentCmp, STUDENT_RECORD_SIZE) != 0) {
            // We have student
            writeReturn = write(com_fd, &student, STUDENT_RECORD_SIZE);
            if (writeReturn < 0) {
                printf(M_ERR_DB_WRITE);
                return ERR_DB_FILE;
            }
